...
def target_session_io_lock(session: str, *, timeout: float = 5.0):
    with _path_lock(
        ...
        timeout=timeout,
        error_message=f"Timed out waiting for target session IO lock: {session}",
    ):
        yield


@contextlib.contextmanager
def inline_host_lock(
    tmux_session: str, host_pane_id: str = "", *, timeout: float = 5.0
):
    with _path_lock(
        inline_host_lock_path(tmux_session, host_pane_id),
        timeout=timeout,
        error_message=f"Timed out waiting for inline host lock: {tmux_session}:{host_pane_id}",
    ):
        yield
