...
def _is_orche_tmux_session(name: str) -> bool:
    ...
    return bool(session_name) and (
        session_name in _known_tmux_sessions()
        or session_name.startswith(f"{TMUX_SESSION}-")
    )
...
def _tmux_has_session(name: str) -> bool:
    ...
    if not session_name:
        return False
    return (
        tmux("has-session", "-t", session_name, check=False, capture=True).returncode
        == 0
    )
...
def list_tmux_sessions() -> List[str]:
    ...
    return [
        ...
        if _is_orche_tmux_session(line.strip())
    ]
...
def _tmux_value_for_pane(pane_id: str, fmt: str) -> str:
    result = tmux(
        "display-message", "-p", "-t", pane_id, fmt, check=False, capture=True
    )
